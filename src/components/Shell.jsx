  ...
  return (
    <div className="dx-scrollport">
      <div className="dx-grain min-h-full w-full max-w-full pb-[env(safe-area-inset-bottom)]">
        <header className="sticky top-0 z-40 w-full border-b border-line/70 bg-[#0b1017]/90 backdrop-blur-md supports-[backdrop-filter]:bg-[#0b1017]/75">
            ...
            >
              {links.map((l) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `min-w-0 rounded-xl px-1 py-2.5 text-center text-[13px] font-medium transition ${
                      isActive
                        ? 'bg-white/10 text-ink'
                        : 'text-ink-soft hover:bg-white/5 hover:text-ink'
                    }`
          ...
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-3 py-8 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div className="flex min-w-0 items-start gap-3 sm:items-center">
              ...
              <p className="min-w-0 text-sm leading-relaxed text-ink-soft">
                <span className="font-display text-ink">DosyaX</span> — teknoloji haberleri ve
                seçilmiş indirimler.
              </p>
            </div>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex min-w-0 items-center gap-2 text-sm font-medium text-[#9fd0ff] hover:underline"
            >
